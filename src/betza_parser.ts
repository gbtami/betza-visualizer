...
export class BetzaParser {
    private readonly atoms: Map<string, {x: number, y: number}> = new Map([
        ['W', {x: 1, y: 0}], ['F', {x: 1, y: 1}], ['D', {x: 2, y: 0}],
        ['N', {x: 2, y: 1}], ['A', {x: 2, y: 2}], ['H', {x: 3, y: 0}],
        ['C', {x: 3, y: 1}], ['Z', {x: 3, y: 2}], ['G', {x: 4, y: 1}]
    ]);

    private readonly compoundAliases: Map<string, string> = new Map([
        ['B', 'F0'], ['R', 'W0'], ['Q', 'W0F0'], ['K', 'W1F1'],
        ['E', 'RN'], ['J', 'AD'], ['M', 'FC']
    ]);
    ...
    public parse(notation: string): Move[] {
        ...
        while (tokenWorklist.length > 0) {
            ...
            if (currentMods.includes('m') && !currentMods.includes('c')) moveType = 'move';
            else if (currentMods.includes('c') && !currentMods.includes('m')) moveType = 'capture';
            else if (currentMods.includes('m') && currentMods.includes('c')) {
                moveType = currentMods.lastIndexOf('c') > currentMods.lastIndexOf('m') ? 'capture' : 'move';
            }

            const hopType: Move['hopType'] = currentMods.includes('p') ? 'p' : currentMods.includes('g') ? 'g' : null;
            ...
            const maxSteps = countStr === '' ? 1 : countStr === '0' ? this.infinityCap : parseInt(countStr);
            const {x: atomX, y: atomY} = this.atoms.get(atom)!;

            const baseDirections = this._getDirections(atomX, atomY);
            const allowedDirections = this._filterDirections(baseDirections, currentMods);

            for (let i = 1; i <= maxSteps; i++) {
                for (const {dx, dy} of allowedDirections) {
                    const move: Move = {
                        x: dx * i, y: dy * i,
                        moveType, hopType, jumpType, atom
                    };
                    moves.push(move);
                }
            }
    ...
    private _getDirections(x: number, y: number): Set<{dx: number, dy: number}> {
        ...
        return new Set(Array.from(directions).map(s => {
            const [dx, dy] = s.split(',').map(Number);
            return {dx, dy};
        }));
    }

    private _filterDirections(directions: Set<{dx: number, dy: number}>, mods: string): Set<{dx: number, dy: number}> {
        if (mods.includes('s')) mods += 'lr';
        if (mods.includes('v')) mods += 'fb';
        const dirMods = mods.split('').filter(c => 'fblr'.includes(c)).join('');
        ...
        const constrainDoubleHorizontal = mods.includes('ll') || mods.includes('rr');
        ...
        for (const {dx, dy} of directions) {
            ...
            if (constrainDoubleVertical && Math.abs(dy) <= Math.abs(dx)) isValid = false;
            if (constrainDoubleHorizontal && Math.abs(dx) <= Math.abs(dy)) isValid = false;
