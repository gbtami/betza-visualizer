...
class BetzaParser:
    ...
    def parse(self, notation: str) -> List[Tuple[int, int, str, Optional[str], str, str]]:
        ...
        while token_worklist:
            ...
            if letter not in self.atoms: continue

            atom, count_str = letter, suffix

            # Determine move_type
            move_type = 'move_capture'
            if 'm' in current_mods and 'c' not in current_mods: move_type = 'move'
            elif 'c' in current_mods and 'm' not in current_mods: move_type = 'capture'
            ...
            jump_type = 'normal'
            if 'n' in current_mods: jump_type = 'non-jumping'
            elif 'j' in current_mods: jump_type = 'jumping'
    ...
    def _filter_directions(self, directions: Set[Tuple[int, int]], mods: str) -> Set[Tuple[int, int]]:
        if 's' in mods: mods += 'lr'
        if 'v' in mods: mods += 'fb'

        dir_mods = ''.join(c for c in mods if c in 'fblr')
        if not dir_mods: return directions
        ...
        for x, y in directions:
            ...
            if constrain_forward and y <= 0: is_valid = False
            if constrain_backward and y >= 0: is_valid = False
            if constrain_left and x >= 0: is_valid = False
            if constrain_right and x <= 0: is_valid = False
            if constrain_double_vertical and abs(y) <= abs(x): is_valid = False
            if constrain_double_horizontal and abs(x) <= abs(y): is_valid = False
